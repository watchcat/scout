...
optionally the [Perplexity Search API](https://docs.perplexity.ai), and
...
- Two search engines merged per query when `PERPLEXITY_API_KEY` is set: on
  the same Dutch shopping query their results overlapped by 2 URLs in 20,
  and each found small shops the other missed
- Searches in the user's local languages as well as English (derived from
  the delivery country, or set explicitly), all in one call
- Cheapest-price requests are answered from a deterministic comparison:
  landed cost (item + shipping) and price per unit, so a 3-pack and a single
