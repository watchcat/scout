...
test('the trip reads as a row of days, and every day of it is accounted for', () => {
  ...
  //
  // The cases live in `day_rows.json` because the printed plan builds the
  // same rows in Rust, and two implementations of one rule with nothing
  // between them is how the two surfaces come to disagree about what a
  // day holds. `trip_pdf.rs` reads this same file.
  const shapeOf = (rows) => rows.map((row) => row.kind === 'free'
    ? `free ${row.days}`
    : `${row.date}:${row.entries.map((e) => /^\d{2}:\d{2}$/.test(e.time) ? `${e.type}@${e.time}` : e.type).join(',')}`)
  const cases = JSON.parse(readFileSync(new URL('./day_rows.json', import.meta.url), 'utf8')).cases
  for (const day of cases) {
    assert.deepEqual(shapeOf(tripDayRows({ items: day.items }, 'en-GB')), day.rows, day.name)
  }

  // What the rows say, which is each side's own business.
  const rows = tripDayRows({ items: cases[0].items }, 'en-GB')
  const at = (date) => rows.find((row) => row.date === date)
  assert.equal(at('2026-09-21').entries[0].text, 'AMS → HKG')
  assert.equal(at('2026-09-21').entries[0].position, 2)
  assert.equal(at('2026-09-22').entries[0].text, 'Lands at HKG')
  assert.equal(at('2026-09-22').entries[0].position, 2, 'the arrival points at the leg it belongs to')
  assert.equal(at('2026-09-22').entries[1].nights, 3)
  assert.equal(at('2026-09-24').label, 'Thu 24')
  assert.equal(at('2026-09-24').entries[0].held, false)
  assert.equal(at('2026-09-21').entries[0].held, true)
  assert.deepEqual(at('2026-09-23').entries, [])
