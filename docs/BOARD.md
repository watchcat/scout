...
- [ ] **Read what a part is when the mail arrives** — the worker decides which parts are decoration from the received record, an endpoint that documents neither `content_disposition` nor `content_id`; the `email.received` webhook is the one source confirmed to carry both. Parse and store them at ingest, so the filtering rests on data we have seen and the retry gap closes with no network call.
