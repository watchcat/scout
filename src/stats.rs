...
mod tests {
    ...
    fn empty_window_reports_no_requests() {
        assert_eq!(
            format_stats(&[], 7, date("2026-07-27"), 1, &no_names()),
    ...
    fn totals_averages_and_ordering() {
        ...
        let out = format_stats(&rows, 2, date("2026-07-27"), 111, &no_names());
    ...
    fn caller_alone_is_marked_you_and_keeps_their_id() {
        ...
        let out = format_stats(&rows, 1, date("2026-07-27"), 42, &named(&[(42, "Alice")]));
    ...
    fn an_unknown_id_still_renders() {
        ...
        let out = format_stats(&rows, 1, date("2026-07-27"), 1, &no_names());
    ...
    fn day_chart_zero_fills_and_scales() {
        ...
        let out = format_stats(&rows, 3, date("2026-07-27"), 1, &no_names());
    ...
    fn tiny_counts_still_show_a_bar() {
        ...
        let out = format_stats(&rows, 2, date("2026-07-27"), 1, &no_names());
